...
2. ./client (pid) "메시지" 이렇게 줬을 때, 메시지 인자 안에 공백문자가 텍스트형식으로 들어가있는걸 다시 공백문자로 바꿔주기. (꼭 해야하는 게 맞나?)
    -> client 메시지 그대로 전달하는 것이므로 그냥 안하기로 함. (시간 나면 해야지)
...
남은거 : 
1. 공백문자 들어가게 만들기
2. signal 관련 함수들 리턴값 살펴보며 에러처리 추가.
3. 노미넷 맞추기 (for문을 while로, 그 외에 노미넷도.)
4. clock() 함수로 아주 세밀한 시간까지 체크해서 과제에 제시된 오버타임을 안넘는지 보기
5. 주석처리한 코드리뷰 버전은 노션에 따로 올리고 여기에는 한줄 주석 지우기
